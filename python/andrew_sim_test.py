...
class BatchedEnvTester:
    ...
    def __init__(self):
        ...
        self._include_debug_sensor = False
        debug_width = 768
        debug_height = 768
        ...
        bsim_config.num_debug_envs = (
            self._num_envs if self._include_debug_sensor else 0
        )
        ...
        bsim_config.episode_set_filepath = "../data/tidy_house.json"
    ...
    def run(self, num_batch_steps):
        ...
        # resets = [5 for _ in range(self._num_envs)]
        ...
        for step_index in tqdm(range(num_batch_steps)):
            self._bsim.start_render()
            self._bsim.wait_render()
            self.save_pixel_observations(step_index, "default")

            # Uncomment if you want to render videos.
            # self.write_videos(step_index)
            ...
            if step_index % episode_len == (episode_len - 1):
                resets = [
                    self.get_next_episode() for _ in range(self._num_envs)
                ]
            else:
                resets = [-1] * self._num_envs
    ...
    def save_pixel_observations(self, step_index, set_name):
        ...
        for env_index in self._envs_to_save:
            ...
            step_dict = env_saved_observations[len(env_saved_observations) - 1]
            if self._include_rgb:
                step_dict[set_name + "_rgb"] = rgb_obs[env_index, ...]
            if self._include_depth:
                step_dict[set_name + "_depth"] = depth_obs[env_index, ...]
            if self._include_debug_sensor:
                step_dict[set_name + "_debug_rgb"] = debug_rgb_obs[
                    env_index, ...
                ]

    def write_videos(self, step_index):
        video_folder = "../videos"
        rank = 0
            ...
            for set_name in ["default"]:
                for obs_type in self._obs_types_to_save:
                    ...
                    vut.make_video(
                        ...
                        video_folder + "/sim_test_env" + str(env_index) + "_"
                        # + str(step_index)
                        # + "_"
                        + obs_name,
